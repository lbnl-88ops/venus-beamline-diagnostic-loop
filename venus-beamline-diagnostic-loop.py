...
while again:
    ...
    if request_csd:
        venus.write({'fcv1_ammeter_stdev':0.})   # set to zero as an indicator csd is happening
    ...
    if venus.read(['emittance_retract_axis']):
        JessicaFunctionRetractAxes()
        # Retract the venus emittance scanner axes
    ...
    if venus.read(['emittance_scan_request']):
        ...
        if venus.read(['fcv1_in']):   # checking if faraday cup is in
            ...
            while(venus.read(['fcv1_in']):  time.sleep(.1)
        ...
        JessicaCallsMagicEmittance( venus.read(['emittance_direction']),
            venus.read(['emittance_position_min']), venus.read(['emittance_position_max']),
            venus.read(['emittance_position_step']), venus.read(['emittance_divergence_min']),
            venus.read(['emittance_divergence_max']), venus.read(['emittance_divergence_step']),
            venus.read(['emittance_keithley_multiplier']), leave_scanner_in)
        ### NOTE: keithley multiplier is an integer:  turn to 10Einteger
        venus.write({'emittance_scan_in_progress':0})
        formatted_time = nowdt.strftime("%Y-%m-%d %H:%M:%S")
        with open(directory+'log','a') as f:
            f.write(f'{formatted_time} emittance scan time = {time.time()-tscanstart:.1f}\n')
